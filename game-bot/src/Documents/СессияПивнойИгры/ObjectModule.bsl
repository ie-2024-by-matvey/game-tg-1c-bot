...
Процедура РассчитатьРаунд(НомерРаунда) Экспорт
	...
	Иначе
		ПоступившиеОтправки = ХодИгры[НомерРаунда - СрокДоставки - 1];
	КонецЕсли;
	...
	Для Сч = 1 По 4 Цикл
		...
		Если Сч = 4 Тогда
			ВходящийЗаказ = ДанныеРаунда.Спрос;
		Иначе
			ВходящийЗаказ = ДанныеРаунда["Заказ" + (Сч + 1)];
		КонецЕсли;
		
		НеобходимоОтправить = ВходящийЗаказ;
		...
		ДанныеРаунда["Поступление" + Сч] = Поступление;
		ДанныеРаунда["ВходящийЗаказ" + Сч] = ВходящийЗаказ;		
